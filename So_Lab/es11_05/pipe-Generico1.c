   ...
   int pidFiglio, status, ritorno;      /* per wait padre */
	...
	{   
		...
		{   
			...
			/* il figlio che e' il produttore comunica al padre la linea letta dal file dopo averla trasformata in stringa (per il protocollo di comunicazione stabilito con il padre */
	...
	while (read(piped[0], &index,sizeof(int)))  /* questo ciclo avra' termine appena il figlio terminera' dato che la read senza piu' scrittore tornera' 0! */
	{ 	
		read(piped[0],mess, index);
